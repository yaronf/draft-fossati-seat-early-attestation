...
and compare the computed value to the attestation binder included in the
...
However a correct TEE is required to verify the binder against the TLS public key associated
with the private key that it holds. This verification, in conjunction with the TEE's
software being endorsed and verified, ensures that relay attacks are prevented.
